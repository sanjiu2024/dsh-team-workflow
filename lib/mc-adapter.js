...
/** §N§ ordinal 前缀：bundle 每轮重打，不属于「可见内容」。
 * 只用于【比较/去重】时剥离；真正落地时保留原文（模型靠它调 ctx_expand）。
 */
const ORDINAL_RE = /^§\d+§[ \t]*/;
 ...
 * 剥掉 §N§。
 *
 * `m` 多行标志是必须的：`textOfMessage` 把 reasoning / text / tool-call 各块的
 * 文本用 `\n` 拼接，而 bundle 只给 **text 块**打 §N§。只锚 `^` 的话，那条
 * ordinal 会落在拼接串的中段（实测下标 603）而剥不掉，于是已存在的 assistant
 * 消息被判成「新增」，再被当作注入块写成 `user/message` —— 这正是 158 条坏
 * 事件、5 个会话加载失败的根因（见 docs/HANDOFF-10）。
 *
 * 代价（已知，接受）：模型文本里恰好**行首**写 `§5§` 时也会被剥。危害有限 ——
 * 比较是两侧对称的（before/after 同一规则，不产生假 diff），唯一真丢内容的位置是
 * 折叠载体写入（`stripOrdinaled`）。要完全避开得按块比较（reasoning 单独比、
 * 只对 text 块剥），那要同时动 `sameVisible` / `normText` / `alignSurface`
 * 三方契约，收益不抵风险。
 */
...
/** 比较用的规范化文本：剥掉 §N§ 和首尾空白。
 * 为什么剥 §N§：序号是 bundle 自己按位置重排的产物，同一段内容在两轮之间
 * 编号会整体移位（前面折掉一段，后面全体 -1）。若不剥，diff 会把每条消息
 * 都判成「变了」，折叠区间永远识别不出来 —— 这正是旧版 landOnSurface
 * 只会 append 的根因。
 */
function normText(message) {
	return stripOrdinal(textOfMessage(message)).trim();
}

/** 可见内容等价：只比 role + 文本（§N§ 已剥离）。
 * bundle 每轮重造消息对象（id/source 都会变），比整个对象会产生假 diff。
 */
function sameVisible(x, y) {
	if ((x?.role ?? "user") !== (y?.role ?? "user")) return false;
	return normText(x) === normText(y);
}

/**
 * 把 bundle 输出的消息数组对齐回**旧数组上的下标**。
 *
 * 用「最长公共前缀 + 最长公共后缀」求中间被换掉的段：
 *   · 前缀/后缀比较时【剥掉 §N§】—— 序号是 bundle 自己按位置重排的产物，
 *     前面折掉一段后面全体 -1。不剥的话每条都判成「变了」，diff 退化，
 *     折叠区间永远识别不出（这正是旧版只会 append 的根因）。
 *   · 中间段 = 旧序列被折叠的 N 条 ↔ 新序列顶上的 M 条。这就是 N→M 折叠，
 *     落地时固定用 N→1（dsh 的 replace 只能 N→1，M>1 的多余部分另行 append）。
 *   · <session-*> 注入块从两侧剔出去，单独作为尾部 append —— dsh surface 无头插。
 *
 * 返回的 start/end 是**旧完整数组的下标**（含注入块），调用方直接拿去映射
 * surface seq，不需要再对齐一次。
 */
export function alignSurface(before, after) {
	const aAll = before ?? [];
	const aHist = [];
	const aIdx = [];
	for (let i = 0; i < aAll.length; i += 1) {
		if (isInjection(aAll[i])) continue;
		aHist.push(aAll[i]);
		aIdx.push(i);
	}

	const bAll = after ?? [];
	const bHist = bAll.filter((m) => !isInjection(m));
	const injections = bAll.filter((m) => isInjection(m));

	// 最长公共前缀
	let pre = 0;
	const maxPre = Math.min(aHist.length, bHist.length);
	while (pre < maxPre && sameVisible(aHist[pre], bHist[pre])) pre += 1;

	// 最长公共后缀（不与前缀重叠）
	let suf = 0;
	const maxSuf = Math.min(aHist.length, bHist.length) - pre;
	while (suf < maxSuf && sameVisible(aHist[aHist.length - 1 - suf], bHist[bHist.length - 1 - suf])) suf += 1;

	const aMidEnd = aHist.length - suf; // exclusive
	const bMidEnd = bHist.length - suf; // exclusive
	const midA = aHist.slice(pre, aMidEnd);
	const midB = bHist.slice(pre, bMidEnd);
	...
	if (midA.length > 0) {
		ops.push({
			...
			midA,
			midB,
			note: `${midA.length} 条 → ${midB.length} 条`,
		});
	} else if (midB.length > 0) {
		// 中间凭空多出消息（surface 无真正的中插，按尾追加处理）
		ops.push({ kind: "append", messages: midB });
	}

	if (injections.length > 0) ops.push({ kind: "append", messages: injections });
	return ops;
}
