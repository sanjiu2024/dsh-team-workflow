...
import { WORKTREE_DEFAULTS, WORKTREE_FIELDS, installWorktree } from "./worktree.js";
...
export function apply(ctx, config = {}) {
	...
	// —— 7e. 子代理 worktree 隔离（REQ-001）——
	// dsh 的子代理没有工作区隔离（`SubagentCapabilities` 里就没有这一项），
	// 并发写代码会互相踩。本模块用 git CLI 做成手动闸门：
	// 建独立 worktree → 各自干活 → 合回主分支。
	// 只用 ctx.tools，不碰 dsh 内部 API —— 这正是它能绕开「import 不了
	// @deepseek-ai/*」的原因。详见 docs/requirements/REQ-001-worktree隔离.md。
	const worktree = installWorktree(ctx, {
		config: layered("team/extensions/worktree.json", WORKTREE_DEFAULTS, WORKTREE_FIELDS, config.worktree),
	});
	...
	ctx.logger.info(
		`[team] dsh-team-workflow 已加载：审计=${auditConfig.enabled} rtk=${rtk.active} lens=${lens.enabled} lens工具=${lensTools.enabled} context7=${context7.enabled} 交接=${handoff.enabled} bash=${bashLinux.enabled} worktree=${worktree.enabled}`,
	);
}
