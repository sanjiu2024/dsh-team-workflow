...
export const WEB_DEFAULTS = {
	...
	/**
	 * 同一查询的缓存窗口。0 = 关掉。
	 * 10 分钟是按 09-25 的真实间隔定的：那天 6 次重复查询的间隔是
	 * 26s / 127s / 293s / 315s / 448s / 2186s。
	 * 5 分钟只拦得住 3 次（那个 315s 的正好卡在外面）；10 分钟拦得住 5 次，
	 * 再往上直到 30 分钟也都是 5 次 —— 所以用 10 分钟：拿一样多的收益，
	 * 陈旧窗口小 3 倍（搜索结果也会过期）。
	 */
	cacheTtlMs: 600_000,
	/** 缓存最多几条。超了先丢最旧的 */
...
/**
 * 给搜索 provider 套一层查询缓存。
 *
 * 为什么加：09-25 那天 13 次 web_search 里有 6 次是重复查询（46%），最密一组只隔
 * 26 秒，而且是**跨会话**重复的——各人各搜一遍同一件事。白花的不只是那次网络等待，
 * 结果还整份再进一次上下文，之后每一步都重新携带它。
 *
 * 效益要说实话：那天真正省下的只有 2 次（「npm view 命令用法」「deepseek」），
 * 放大后合计 ~11k 字符。因为另外 4 组的首次结果都是 158 字符的空结果（那天
 * 后端被反爬拦着），而空结果按设计不缓存。所以这层缓存的价值取决于后端通不通：
 * 后端正常时搜一次结果有 1.6k~8.5k 字符，那时候拦下一次重复才真值钱。
 *
 * 为什么进程级缓存真能拦住这些重复：dsh 的 `ctx.sessions` 是个 Service（一个 ctx 一个
 * 实例），里面用 Map 持着**所有活着的会话** —— 一个进程同时在服务多个会话，而
 * `installWebSearch` 每次插件加载只跑一次，所以一份 provider 实例管着全部会话。
 * （那天日志里也有 22 次「2 秒内 A→B→C 三方交替」，同一个进程无疑。）
 *
 * 为什么做成装饰器而不是改 search 本体：本体是已过审查的代码，往里塞缓存会把
 * 整段重缩进、把 diff 从 60 行撞成 180 行，审查时看不出真正改了啥。
 *
 * 缓存 key 是「影响输出的全部输入」——同一组词限 5 条和限 8 条是两个不同请求，
 * 共用一条缓存会让模型拿到错的结果。
 *
 * 只缓存**已完成的成功结果**，不缓存 in-flight 的 Promise。存 Promise 看着能
 * 顺手合并并发重复，但这个缓存是进程级的（跨会话共享）：那条 Promise 绑的是
 * 第一个调用方的 signal，A 会话一取消，命中同一缓存的 B 会话会收到一个不是它
 * 引起的取消错误。而 09-25 的真实日志里同一会话内重复查询是 0 组、并发重复 0 组，
 * 换来的收益是空的。
 *
 * 不缓存就安全：缓存里存的是 provider 的返回值（对象引用），而下游
 * 官方的 `capSources` 用 `slice()` 另建新对象、`mergeSearchResults` 也是重建，
 * 都不原地改它 —— 所以多个会话共享同一条缓存不会被彼此污染。
 * 哪天要在下游原地改 sources，这里就得改成存深拷贝。
 */
function withSearchCache(provider, config, log) {
	const ttlMs = config.cacheTtlMs ?? WEB_DEFAULTS.cacheTtlMs;
	const maxEntries = config.cacheMaxEntries ?? WEB_DEFAULTS.cacheMaxEntries;
	...
	return {
		...
		async search(request, signal) {
			// key 里直接用 request.maxResults 原值，不在这里再写一份默认值：
			// 本体里有自己的 `?? 8`，这里若也写一份，哪天本体改了默认值，
			// 这个 key 就会把「本体给 10 条」和「本体给 8 条」两个请求归成一条。
			...
			if (hit && Date.now() - hit.at < ttlMs) {
				log(`[team:web] 命中查询缓存：「${request.query}」`);
				return hit.value;
			}

			// 抛错会直接冒出去，走不到下面的 cache.set —— 失败不进缓存。
			const result = await provider.search(request, signal);
			// 两个后端都挂时 search 是**正常返回**一个空结果（带失败原因），不走 catch。
			// 这种空结果同样不能缓存 —— 后端抖一下就把它钉住整个 TTL。
			if (result.sources.length > 0) {
				cache.set(key, { at: Date.now(), value: result });
				// 先塞后裁：Map 保证迭代顺序即插入顺序，最旧的在最前面。
...
export function makeSearchProvider(ctx, config, log = () => {}) {
	...
	return withSearchCache(provider, config, log);
}
